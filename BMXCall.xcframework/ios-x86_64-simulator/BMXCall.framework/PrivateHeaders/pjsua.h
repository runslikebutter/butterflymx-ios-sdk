 ...
 */
 ...
 //custom - added by Ognjen

typedef struct adaptive_settings
{
	int InternetDownTimeout;
	int AudioExtremeDelay;
	int AudioExtremeJitter;
	int AudioExtremeLossRate;
	int AudioSignificantDelay;
	int AudioSignificantJitter;
	int AudioSignificantLossRate;
	int VideoExtremeDelay;
	int VideoExtremeJitter;
	int VideoExtremeLossRate;
	int VideoSignificantDelay;
	int VideoSignificantJitter;
	int VideoSignificantLossRate;
	int AdaptiveFrequency;
	double BadSipConnectionFactor;
} a_settings;


void start_qos(unsigned call_id);
void set_adaptive_settings_and_pointer_to_bad_sip_connection(a_settings * adaptive, void* bad_sip_connection);
void set_adaptive_settings(a_settings * adaptive);
void end_qos();

PJ_DECL(pj_status_t) pjsua_call_make_call_qos(pjsua_acc_id acc_id,
	const pj_str_t *dst_uri,
	const pjsua_call_setting *opt,
	void *user_data,
	const pjsua_msg_data *msg_data,
	pjsua_call_id *p_call_id,
	a_settings *settings,
	void* bad_sip_connection);

/**/
PJ_DECL(pj_status_t) pjsua_call_answer2_qos(pjsua_call_id call_id,
	const pjsua_call_setting *opt,
	unsigned code,
	const pj_str_t *reason,
	const pjsua_msg_data *msg_data,
